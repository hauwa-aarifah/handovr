...
class AmbulanceJourneyGenerator:
    ...
    def generate_incidents(self, start_date, end_date, hourly_rate=15, rate_multiplier=1.0):
        """
        Generate ambulance incidents across London
        ...
        end_date : datetime
            End date/time for the simulation
        hourly_rate : int, optional
            Average number of incidents per hour across London (adjusted for accuracy)
        rate_multiplier : float, optional
            Multiplier to adjust the incident rate
    ...
    def select_hospitals_for_incidents(self, incidents_with_ambulances, hospital_data=None):
        """
        Select hospitals for incidents based on CURRENT ambulance routing practices
        (i.e., usually nearest appropriate hospital, not optimized selection)
        ...
        incidents_with_ambulances : DataFrame
            Incidents with assigned ambulances
        hospital_data : DataFrame, optional
            Not used in current system simulation (kept for compatibility)
        ...
        print(f"Assigning hospitals to {len(incidents_with_hospitals)} incidents based on proximity...")
        
        # Pre-calculate all hospital distances for efficiency
        hospital_coords_list = [(h_id, self.hospital_loc_map[h_id]) for h_id in self.hospital_loc_map.keys()]
        
        # Process each incident
        for idx, incident in incidents_with_hospitals.iterrows():
            if idx % 5000 == 0:
                print(f"  Processing incident {idx}...")
            ...
            for hospital_id, hospital_loc in hospital_coords_list:
                distance = geodesic(incident_loc, hospital_loc).kilometers
                hospital_type = self.hospital_type_map[hospital_id]
                
                # Current system: Basic filtering only
                # Type 3 (UTC) cannot handle high severity
                if hospital_type == "Type 3" and severity >= 7:
                    continue
                    
                # Specialty hospitals only for specific cases
                if hospital_type == "Type 2":
                    if "ORTHOPAEDIC" in hospital_id and incident_type not in ["Trauma", "Fall"]:
                        continue
                    elif "EYE" in hospital_id and incident_type not in ["Other Medical", "Trauma"]:
                        continue
                
                hospital_distances.append((hospital_id, distance, hospital_type))
            
            # Sort by distance (current system behavior)
            hospital_distances.sort(key=lambda x: x[1])
            
            # Current system logic: Usually take the nearest appropriate hospital
            if severity >= 8:  # Critical cases
                # Always nearest appropriate hospital
                chosen_hospital, distance, _ = hospital_distances[0]
            else:
                # Still usually nearest, but with small chance of variation
                # (representing paramedic discretion, patient preference, etc.)
                if len(hospital_distances) > 1 and np.random.random() < 0.15:  # 15% chance
                    # Occasionally go to second nearest
                    chosen_hospital, distance, _ = hospital_distances[1]
                else:
                    # Usually nearest
                    chosen_hospital, distance, _ = hospital_distances[0]
            ...
            transport_time = self.calculate_journey_times(
                incident_loc,
                self.hospital_loc_map[chosen_hospital],
                time_period,
                weather_condition
            )
    ...
    def generate_ambulance_journeys(self, start_date, end_date, hospital_data=None, hourly_rate=15):
        """
        End-to-end generation of ambulance journeys
...
def main():
    ...
    parser.add_argument("--start-date", default="2024-10-01", help="Start date (YYYY-MM-DD)")
    parser.add_argument("--end-date", default="2024-12-31", help="End date (YYYY-MM-DD)")
    parser.add_argument("--hourly-rate", type=int, default=15, help="Average incidents per hour")
    parser.add_argument("--output-dir", default="data/raw", help="Output directory")
    args = parser.parse_args()
