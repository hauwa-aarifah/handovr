...
class LondonHospitalSimulation:
    ...
    def __init__(self, start_date="2024-10-01", end_date="2024-12-31"):
        ...
        # Define major Type 1 hospitals based on actual NHS data
        self.major_type1_hospitals = [
            "BARTS HEALTH NHS TRUST",
            "IMPERIAL COLLEGE HEALTHCARE NHS TRUST",
            "KING'S COLLEGE HOSPITAL NHS FOUNDATION TRUST",
            "GUY'S AND ST THOMAS' NHS FOUNDATION TRUST",
    ...
    def _determine_hospital_size(self, name, hospital_type):
        ...
        # Major teaching hospitals are large
        if name in self.major_type1_hospitals[:8]:  # Top 8 are largest
            return "large"
        else:
            return "medium"
    ...
    def _initialize_london_hospitals(self):
        ...
        # Total attendances for Q4 should be ~350,000 across all hospitals
        # This is roughly 130 attendances per hour across all hospitals
        # Distribute based on hospital size and type
        
        for hospital_name in LONDON_HOSPITALS:
            hospital_type = self._determine_hospital_type(hospital_name)
            
            if hospital_type == "Type 1":
                if hospital_name in self.major_type1_hospitals[:8]:  # Large Type 1
                    base_arrivals = np.random.uniform(8, 12)  # Adjusted for accuracy
                    bed_capacity = np.random.uniform(120, 160)
                    staff_day = np.random.uniform(45, 55)
                    staff_night = np.random.uniform(30, 40)
                    treatment_rooms = np.random.uniform(25, 35)
                    icu_beds = np.random.uniform(20, 30)
                    # Create bimodal base performance
                    if np.random.random() < 0.3:
                        base_performance = np.random.normal(0.58, 0.02)
                    else:
                        base_performance = np.random.normal(0.71, 0.02)
                else:  # Medium Type 1
                    base_arrivals = np.random.uniform(5, 8)
                    bed_capacity = np.random.uniform(80, 120)
                    staff_day = np.random.uniform(35, 45)
                    staff_night = np.random.uniform(22, 30)
                    treatment_rooms = np.random.uniform(18, 25)
                    icu_beds = np.random.uniform(12, 20)
                    # Bimodal performance
                    if np.random.random() < 0.35:
                        base_performance = np.random.normal(0.60, 0.02)
                    else:
                        base_performance = np.random.normal(0.69, 0.02)
            elif hospital_type == "Type 2":
                base_arrivals = np.random.uniform(3, 5)
                bed_capacity = np.random.uniform(20, 40)
                staff_day = np.random.uniform(18, 28)
                staff_night = np.random.uniform(10, 18)
                treatment_rooms = np.random.uniform(12, 18)
                icu_beds = 0
                base_performance = np.random.normal(0.88, 0.04)
            else: # Type 3
                base_arrivals = np.random.uniform(4, 7)
                # UTCs have treatment spaces, not traditional beds
                # Model as "treatment capacity" rather than bed count
                bed_capacity = np.random.uniform(8, 15)  # Treatment spaces/chairs
                staff_day = np.random.uniform(12, 22)
                staff_night = np.random.uniform(6, 12)
                treatment_rooms = np.random.uniform(5, 8)
                icu_beds = 0
                base_performance = np.random.normal(0.97, 0.008)
                target_occupancy = np.random.uniform(0.65, 0.85)  # UTCs typically 65-85% occupied
            ...
            hospital_configs[hospital_name] = {
                ...
                "region": "London",
                "base_arrivals": base_arrivals,
                "bed_capacity": bed_capacity,  # This represents treatment spaces
                "staff_day": staff_day,
                "staff_night": staff_night,
                "treatment_rooms": treatment_rooms,
                "icu_beds": icu_beds,
                # "base_performance": np.clip(base_performance, 0.3, 0.99)
                "base_performance": np.clip(base_performance, 0.3 if hospital_type != "Type 3" else 0.94, 0.99)
            }

            # else:  # Type 3
            #     base_arrivals = np.random.uniform(4, 7)  # Increased for Type 3
            #     bed_capacity = np.random.uniform(15, 25)
            #     staff_day = np.random.uniform(12, 22)
            #     staff_night = np.random.uniform(6, 12)
            #     treatment_rooms = np.random.uniform(8, 12)
            #     icu_beds = 0
            #     base_performance = np.random.normal(0.965, 0.015)
            
            # hospital_configs[hospital_name] = {
            #     "type": hospital_type,
            #     "size": self._determine_hospital_size(hospital_name, hospital_type),
            #     "region": "London",
            #     "base_arrivals": base_arrivals,
            #     "bed_capacity": bed_capacity,
            #     "staff_day": staff_day,
    ...
    def generate_arrival_pattern(self, base_rate, timestamp, hospital_type):
        ...
        if hospital_type == "Type 1":  # Major A&E
            ...
            # UTCs have grown significantly according to NHS data
            base_rate *= 1.3  # Increase UTC volumes
            if 8 <= hour <= 18:
                hourly_mult = 1.4  # Stronger daytime usage in London
            else:
                hourly_mult = 0.5
    ...
    #     # Length of stay multiplier based on severity and hospital type
    #     if hospital_type == "Type 1":
    #         # Type 1 has longer stays (hours to days)
    #         los_multiplier = np.clip(severity / 4, 0.6, 2.2)
    #         base_los_hours = 4
    #     elif hospital_type == "Type 2":
    #         # Specialty hospitals often have planned pathways
    ...
    #         base_los_hours = 3
    #     else:  # Type 3 - UTC
    #         # UTC has very short stays (usually under 2 hours)
    #         los_multiplier = np.clip(severity / 8, 0.2, 0.5)
    #         base_los_hours = 1.5  # Much shorter base stay
    ...
    #     for t in range(len(arrivals)):
    #         # Calculate duration based on severity and month
    #         stay_duration = int(base_los_hours * los_multiplier[t])
    #         end_idx = min(t + stay_duration, len(arrivals))
    #         current_patients[t:end_idx] += arrivals[t]
        
    #     # Calculate occupancy differently for UTC
    #     if hospital_type == "Type 3":
    #         # For UTC, occupancy represents utilization of treatment spaces
    #         # Since patients cycle through quickly, we need to account for throughput
    #         hourly_throughput = capacity / 2  # Each space can handle ~2 patients per hour
    #         occupancy = current_patients / hourly_throughput
    #     else:
    #         # Traditional bed occupancy for Type 1/2
    #         winter_boost = 1.0
    #         if month == 10:
    #             winter_boost = 1.03
    #         elif month == 11:
    ...
    #         occupancy = current_patients / (capacity * winter_boost)
        
    #     # Clip occupancy with realistic ceiling
    #     max_occupancy = 1.12 if hospital_type == "Type 1" else 1.0
    #     if month == 12 and hospital_type == "Type 1":
    #         max_occupancy = 1.18
            
    #     return np.clip(occupancy, 0, max_occupancy)
    ...
    def calculate_occupancy(self, arrivals, severity, capacity, timestamps, hospital_type):
        ...
        # Calculate occupancy differently for UTC
        if hospital_type == "Type 3":
            # For UTC, use a simpler approach based on patient flow
            # Assume each treatment space can handle 1 patient at a time
            # But they cycle through more quickly
            
            # Calculate rolling average of patients to smooth out the data
            window_size = min(4, len(current_patients))  # 4-hour rolling average
            if window_size > 0:
                rolling_patients = pd.Series(current_patients).rolling(
                    window=window_size, min_periods=1, center=True
                ).mean().values
            else:
                rolling_patients = current_patients
            
            # UTC occupancy is based on current patients vs treatment spaces
            # But we need to account for the fact that UTCs operate differently
            occupancy = rolling_patients / capacity
            
            # Apply a scaling factor because UTCs report occupancy differently
            # They consider a space "occupied" even during patient processing
            occupancy = occupancy * 2.5  # Scaling factor to match realistic UTC occupancy
            ...
            if month == 10:
                ...
                winter_boost = 1.10
            baseline_occupancy = 0.75  # Hospitals are never empty
            occupancy = baseline_occupancy + (0.25 * current_patients / capacity)
            # occupancy = current_patients / (capacity * winter_boost)
        ...
        if hospital_type == "Type 1":
            ...
            max_occupancy = 0.95  # UTCs rarely exceed 95%
    ...
    def calculate_four_hour_performance(self, occupancy, staff_levels, hospital_type, base_performance, timestamps):
        ...
        # NHS monthly performance targets
        monthly_performance = {
            10: {"Type 1": 0.571, "Type 3": 0.965, "Type 2": 0.88},
            11: {"Type 1": 0.559, "Type 3": 0.961, "Type 2": 0.87},
            12: {"Type 1": 0.544, "Type 3": 0.958, "Type 2": 0.85}
        }
        
        # Get target performance for this hospital type and month
        target_performance = monthly_performance.get(month, {}).get(hospital_type, base_performance)
        
        # For Type 1, maintain bimodal distribution while hitting monthly average
        if hospital_type == "Type 1":
            # Determine if this is a struggling or performing hospital
            if base_performance < 0.55:  # Struggling hospital
                hospital_performance = np.random.normal(target_performance - 0.05, 0.03)
            else:  # Better performing hospital
                hospital_performance = np.random.normal(target_performance + 0.08, 0.03)
        else:
            hospital_performance = target_performance
        
        # Occupancy effect - higher occupancy reduces performance
        if hospital_type == "Type 1":
            occupancy_factor = np.clip(1.0 - (occupancy - 0.85) * 1.5, 0.70, 1.0)  # Was 0.82 threshold and 2.2 multiplier
        else:
            occupancy_factor = np.clip(1.0 - (occupancy - 0.82) * 2.2, 0.65, 1.0)
        ...
        # Staff level effect
        target_staff = np.mean(staff_levels)
        staff_factor = np.clip(staff_levels / target_staff, 0.80, 1.1)
        ...
        # Calculate performance for each hour
        performance = hospital_performance * occupancy_factor * staff_factor * time_factors
        ...
        # Ensure within realistic bounds for London
        if hospital_type == "Type 1":
            # Type 1 performance bounds to create proper distribution
            return np.clip(performance, 0.45, 0.75)
        elif hospital_type == "Type 2":
            return np.clip(performance, 0.75, 0.94)
        else:  # Type 3
            return np.clip(performance, 0.955, 0.985)

    def calculate_waiting_times(self, performance, occupancy, staff_levels, severity, timestamps, hospital_type):
        """Calculate waiting times with London-specific patterns matching NHS data"""
        month = timestamps[0].month if len(timestamps) > 0 else 10
        
        # NHS data shows specific 12+ hour wait percentages
        twelve_hour_rates = {
            10: 0.085,  # 8.5% in October
            11: 0.091,  # 9.1% in November
            12: 0.107   # 10.7% in December
        }
        ...
        for i in range(len(performance)):
            if hospital_type == "Type 1":
                # Use the actual performance value for this hour
                within_4hr_prob = performance[i]  # This should already be ~0.55-0.57
                twelve_hour_prob = twelve_hour_rates.get(month, 0.09)
                
                rand = np.random.random()
                
                if rand < within_4hr_prob:
                    # Within 4 hours - most patients
                    waiting_time = np.random.triangular(15, 120, 220)
                elif rand > (1 - twelve_hour_prob):
                    # 12+ hour wait - small percentage
                    waiting_time = np.random.uniform(720, 1440)
                else:
                    # 4-12 hours - remaining patients
                    waiting_time = np.random.uniform(240, 720)
        
        # for i in range(len(performance)):
        #     if hospital_type == "Type 1":
        #         # Determine outcome category for this patient
        #         rand = np.random.random()
        #         twelve_hour_rate = twelve_hour_rates.get(month, 0.09) * 0.4 
                
        #         # Calculate thresholds
        #         within_4hr_threshold = performance[i]
        #         over_12hr_threshold = 1 - twelve_hour_rate
                
        #         if rand < within_4hr_threshold:
        #             # Within 4 hours - triangular distribution
        #             waiting_time = np.random.triangular(15, 120, 220)
        #         elif rand > over_12hr_threshold:
        #             # 12+ hour wait
        #             waiting_time = np.random.uniform(720, 1440)  # 12-24 hours
        #         else:
        #             # 4-12 hours
        #             waiting_time = np.random.uniform(240, 720)
                ...
                # Type 2 hospitals
                if np.random.random() < performance[i]:
                    waiting_time = np.random.triangular(10, 90, 220)
                else:
                    waiting_time = np.random.uniform(220, 480)
                    
            else:  # Type 3
                # Type 3 hospitals have much better performance
                if np.random.random() < performance[i]:
                    waiting_time = np.random.triangular(10, 60, 120)
                else:
                    waiting_time = np.random.uniform(120, 240)
            
            # Adjust based on occupancy and staff
            occupancy_factor = np.clip(1 + (occupancy[i] - 0.8) * 2.2, 1.0, 2.2)
            staff_factor = np.clip(1 + (1 - staff_levels[i]/np.mean(staff_levels)), 0.8, 1.7)
            
            # Severity impact
            severity_factor = 1.0
            if severity[i] >= 8:  # Critical cases
                severity_factor = 0.65  # Get treated faster
            elif severity[i] <= 2:  # Very minor cases
                severity_factor = 1.3  # May wait longer
            ...
            if hour >= 22 or hour <= 6:
                time_factor *= 1.4  # Worse overnight
            if dow == 0 and 8 <= hour <= 12:
                time_factor *= 1.3  # Monday morning pressure
            if dow >= 5:
                time_factor *= 1.25  # Weekend effect
            if month == 12 and dow >= 5:
                time_factor *= 1.2  # December weekends worse
            
            # Apply all factors
            adjusted_time = waiting_time * occupancy_factor * staff_factor * severity_factor * time_factor
            
            # Add some random variation
            noise = np.random.exponential(20)
            final_time = adjusted_time + noise
            
            waiting_times.append(final_time)
        
        # Ensure realistic bounds
        waiting_times = np.array(waiting_times)
        if hospital_type == "Type 1":
            return np.clip(waiting_times, 0, 1800)  # Cap at 30 hours max
        else:
            return np.clip(waiting_times, 0, 720)  # Cap at 12 hours max
    ...
    def generate_hospital_data(self, hospital_id, config, ambulance_data=None):
        ...
        # Track patients with 12+ hour waits
        patients_over_12hr = np.zeros(num_records)
        
        for i in range(num_records):
            if config["type"] == "Type 1" and ambulance_arrivals[i] > 0:
                # For each hour, determine how many patients waited 12+ hours
                # based on the performance and month
                month = timestamps[i].month
                twelve_hour_rate = self.q4_2024_stats[
                    {10: "oct", 11: "nov", 12: "dec"}[month]
                ]["over_12hr_delays_proportion"]
                
                # Apply some variation
                hour_rate = twelve_hour_rate * np.random.normal(1.0, 0.1)
                hour_rate = np.clip(hour_rate, 0, 0.2)  # Cap at 20%
                
                patients_over_12hr[i] = np.round(ambulance_arrivals[i] * hour_rate)
        ...
        return pd.DataFrame({
            ...
            "Ambulance_Handover_Delay": handover_delays.astype(int),
            "Patient_Waiting_Time_Minutes": waiting_times.astype(int),
            "Patients_Over_12hr": patients_over_12hr.astype(int),
            "Four_Hour_Performance": four_hour_performance,
            "A&E_Bed_Occupancy": occupancy,
            "Patient_Severity_Score": severity_scores,
            "Staff_Levels": staff_levels.astype(int),
    ...
    def validate_against_nhs_stats(self, synthetic_data):
        """Validate synthetic data against known NHS statistics"""
        
        # Expected values from NHS data
        expected_stats = {
            10: {
                "type1_4hr_performance": 0.571,
                "type3_4hr_performance": 0.965,
                "12hr_wait_percentage": 0.085
            },
            11: {
                "type1_4hr_performance": 0.559,
                "type3_4hr_performance": 0.961,
                "12hr_wait_percentage": 0.091
            },
            12: {
                "type1_4hr_performance": 0.544,
                "type3_4hr_performance": 0.958,
                "12hr_wait_percentage": 0.107
            }
        }
        ...
        for month in [10, 11, 12]:
            month_data = synthetic_data[synthetic_data['Month'] == month]
            
            # # Check Type 1 performance
            # type1_data = month_data[month_data['Hospital_Type'] == 'Type 1']
            # actual_type1_perf = (type1_data['Patient_Waiting_Time_Minutes'] <= 240).mean()
            # expected_type1_perf = expected_stats[month]["type1_4hr_performance"]
            
            # # Check Type 3 performance
            # type3_data = month_data[month_data['Hospital_Type'] == 'Type 3']
            # actual_type3_perf = (type3_data['Patient_Waiting_Time_Minutes'] <= 240).mean()
            # expected_type3_perf = expected_stats[month]["type3_4hr_performance"]

            # Check Type 1 performance
            type1_data = month_data[month_data['Hospital_Type'] == 'Type 1']
            # Use weighted average of Four_Hour_Performance instead
            type1_attendances = type1_data['Ambulance_Arrivals'].sum()
            actual_type1_perf = np.average(
                type1_data['Four_Hour_Performance'], 
                weights=type1_data['Ambulance_Arrivals']
            ) if type1_attendances > 0 else 0
            expected_type1_perf = expected_stats[month]["type1_4hr_performance"]
            
            # Check Type 3 performance
            type3_data = month_data[month_data['Hospital_Type'] == 'Type 3']
            type3_attendances = type3_data['Ambulance_Arrivals'].sum()
            actual_type3_perf = np.average(
                type3_data['Four_Hour_Performance'], 
                weights=type3_data['Ambulance_Arrivals']
            ) if type3_attendances > 0 else 0
            expected_type3_perf = expected_stats[month]["type3_4hr_performance"]
            
            # Check 12+ hour waits
            # actual_12hr = (type1_data['Patient_Waiting_Time_Minutes'] > 720).mean()
            if 'Patients_Over_12hr' in type1_data.columns:
                total_over_12hr = type1_data['Patients_Over_12hr'].sum()
                total_type1_patients = type1_data['Ambulance_Arrivals'].sum()
                actual_12hr = total_over_12hr / total_type1_patients if total_type1_patients > 0 else 0
            else:
                # Fallback if column doesn't exist
                actual_12hr = (type1_data['Patient_Waiting_Time_Minutes'] > 720).mean()
            expected_12hr = expected_stats[month]["12hr_wait_percentage"]
            
            validation_results.append({
                "Month": month,
                "Type1_4hr_Actual": actual_type1_perf,
                "Type1_4hr_Expected": expected_type1_perf,
                "Type1_4hr_Diff": abs(actual_type1_perf - expected_type1_perf),
                "Type3_4hr_Actual": actual_type3_perf,
                "Type3_4hr_Expected": expected_type3_perf,
                "Type3_4hr_Diff": abs(actual_type3_perf - expected_type3_perf),
                "12hr_Wait_Actual": actual_12hr,
                "12hr_Wait_Expected": expected_12hr,
                "12hr_Wait_Diff": abs(actual_12hr - expected_12hr)
            })
            
            print(f"\nMonth {month} Validation:")
            print(f"  Type 1 4hr performance: {actual_type1_perf:.3f} (expected: {expected_type1_perf:.3f}, diff: {abs(actual_type1_perf - expected_type1_perf):.3f})")
            print(f"  Type 3 4hr performance: {actual_type3_perf:.3f} (expected: {expected_type3_perf:.3f}, diff: {abs(actual_type3_perf - expected_type3_perf):.3f})")
            print(f"  12+ hour waits: {actual_12hr:.3f} (expected: {expected_12hr:.3f}, diff: {abs(actual_12hr - expected_12hr):.3f})")
    ...
    def generate_monthly_summary(self, data):
        ...
        for month in [10, 11, 12]:  # October, November, December
            ...
            type1_performance = np.average(
                type1_data['Four_Hour_Performance'], 
                weights=type1_data['Ambulance_Arrivals']
            ) if type1_attendances > 0 else 0
            
            type2_performance = np.average(
                type2_data['Four_Hour_Performance'], 
                weights=type2_data['Ambulance_Arrivals']
            ) if type2_attendances > 0 else 0
            
            type3_performance = np.average(
                type3_data['Four_Hour_Performance'], 
                weights=type3_data['Ambulance_Arrivals']
            ) if type3_attendances > 0 else 0
            
            # Overall performance
            total_performance = np.average(
                month_data['Four_Hour_Performance'],
                weights=month_data['Ambulance_Arrivals']
            ) if total_attendances > 0 else 0
            # Calculate admissions (estimated as ~28.5% of Type 1 attendances)
            emergency_admissions = int(type1_attendances * 0.285)
            ...
            delays_over_12hr = int(type1_attendances * twelve_hour_rates.get(month, 0.09))
...
def main():
    ...
    # Validate against NHS statistics
    print("\nValidating against NHS statistics...")
    validation_results = simulator.validate_against_nhs_stats(synthetic_data)
    validation_results.to_csv('data/raw/validation_results.csv', index=False)
    
    # Generate monthly summary
    print("\nGenerating monthly summary statistics...")
    monthly_summary = simulator.generate_monthly_summary(synthetic_data)
    monthly_summary.to_csv('data/raw/london_q4_2024_monthly_summary.csv', index=False)
    
    # Compare with actual NHS statistics
    print("\nComparing with official NHS statistics...")
    comparison = simulator.compare_with_nhs_stats(monthly_summary)
    comparison.to_csv('data/raw/london_q4_2024_nhs_comparison.csv', index=False)
